...
contract ICOINSTALLMENTGAURAV is Ownable { 
    ...
    function createNewICO(address _token, uint256 _startTime, uint256 _endTime, uint256 _amount, uint256 _pricePerToken) public onlyOwner {
        ...
        ERC20(_token).transferFrom(msg.sender, address(this), (_amount * (10 ** tokenDecimals(ICOID))));
    ...
    function returnICOToken(uint256 _icoID) public onlyOwner {
        IERC20(Token[_icoID]).transfer(owner(), (tokenBalance(_icoID)+(ICOInstallmentToken[_icoID] * (10 ** tokenDecimals(_icoID)))));
        installmentTotalToken -= ICOInstallmentToken[_icoID];
        ICOInstallmentToken[_icoID] = 0;
    }
    ...
    function buyToken (uint256 _icoID) external payable checkNotOwner checkICO(_icoID) checkStartTime(_icoID)  checkValue checkToken(_icoID) checkEndTime(_icoID)  {
        ...
        IERC20(Token[_icoID]).transfer(msg.sender, (newTokenAmount * (10 ** tokenDecimals(_icoID))));
        installmentMap[_icoID][msg.sender].installmentToken +=  newTokenAmount;
        installmentMap[_icoID][msg.sender].installmentTime =  (block.timestamp+300);
    ...
    function GetInstallment(uint _icoID) public checkNotOwner checkICO(_icoID) checkInstallment(_icoID) {
        ...
        IERC20(Token[_icoID]).transfer(msg.sender, (instamt * (10 ** tokenDecimals(_icoID))));
    ...
    function sellToken (uint256 _icoID, uint _tokenAmount ) checkNotOwner external checkICO(_icoID) checkTokenBalance(_icoID,_tokenAmount) { 
        ...
        require( allowanceToken >= (_tokenAmount * (10 ** tokenDecimals(_icoID))), "give allowance first "); 
        IERC20(Token[_icoID]).transferFrom(msg.sender , address(this), (_tokenAmount * (10 ** tokenDecimals(_icoID))));
    ...
    function tokenDecimals( uint256 _icoID) public view returns(uint256) {
        uint256 decimal = ERC20(Token[_icoID]).decimals() ;
        return decimal;
    }

    function tokenBalance(uint256 _icoID) public  view returns ( uint ) { 
        uint balance = IERC20(Token[_icoID]).balanceOf(address(this));
        return (balance-(ICOInstallmentToken[_icoID] * (10 ** tokenDecimals(_icoID))));
    }
